...
Hub = {}
Hub.Title = "namdz Hub" .. (getgenv().Premium and " [Premium]" or getgenv().Developer and " [Developer]" or " [Beta]")
Hub.Game = "BloxFruits"

local Fluent = loadstring(game:HttpGet("https://raw.githubusercontent.com/namdzvclhehehe-droid/NamdzKaitun/refs/heads/main/togge.txt"))()

local Window = Fluent:CreateWindow({
    Title = Hub.Title,
    SubTitle = "by nam sai đẹp triêu",
    TabWidth = 160,
...
if CurrnetExecutor == "Wave" or CurrnetExecutor == "AWP" or CurrnetExecutor == "Posstasium" then
    Notify("namdz Hub","Your currnently executor support all function, some function will work good", 3)
else
    Notify("namdz Hub","Your currnently executor not support all function, some function will not work", 3)
end
...
module.Save = function()
    if not isfolder("namdz Hub") then
        makefolder("namdz Hub")
    end
    if not isfolder("namdz Hub/Settings") then
        makefolder("namdz Hub/Settings")
    end
    writefile("namdz Hub/Settings/" .. Hub.Game .. "-" .. plr.Name .. ".json", HttpService:JSONEncode(Settings))
end
...
module.Read = function()
    p1, p2 = pcall(function()
        ...
        return HttpService:JSONDecode(readfile("namdz Hub/Settings/" .. Hub.Game .. "-" .. plr.Name .. ".json"))
    end)
    if p1 then
        return p2
    else
...
local SelectMasteryType = FarmTab:AddDropdown("Select Mastery Type", {
    ...
    Callback = function(v)
        ...
        if v == "Gun Mastery" then
            Notify("namdz Hub","ENG : You want click when mobs low\nVN : Ban can tu click khi mob yeu mau",7)
        end
        module.Save()
    end
})
